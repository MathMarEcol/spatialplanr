...
```{r, fig.width=9}
(ggPU <- splnr_plot_PUs(PUs, landmass)) # Plot Planning Units
```
...
datEx_species_bin <- spDataFiltered %>%
  dplyr::as_tibble() %>%
  dplyr::mutate(dplyr::across(-dplyr::any_of(c("cellID", "geometry")), # Don't apply to geometry and cellID
...
```{r, fig.width=9}
(ggFeature1 <- splnr_plot_binFeature(datEx_species_bin, datEx_species_bin$Chelonia_mydas, PlanUnits = PUs, landmass))
...
```{r, fig.width=9}
(ggFeature <- splnr_plot_binFeature(datEx_species_bin, datEx_species_bin$Megaptera_novaeangliae, PlanUnits = PUs, landmass = landmass))
...
(ggCost <- splnr_plot_cost(out_sf, Cost_name = "Cost_None", plotTitle = "Cost (Area)"))
```
...
```{r, fig.width=9}
(ggSoln <- splnr_plot_Solution(datEx_soln, PUs, landmass))
```
...
```{r, fig.width=9}
(ggCostOverlay <- splnr_plot_costOverlay(soln = datEx_soln, Cost = NA, Cost_name = "Cost_None", landmass))
```
...
dfTarget <- splnr_prepTargetData(datEx_soln, datEx_problem, datEx_species_bin, Dict = Dict, Category = NA,
                                 dataCol = "Scientific",
...
```{r, fig.width=9, fig.height = 10}
ggFerrier <- splnr_plot_impScoreFerrierPlot(datEx_soln, datEx_problem)
ggRWR <- splnr_plot_impScoreRWRPlot(datEx_soln, datEx_problem)
...
Sometimes it can be useful to have a direct comparison of two spatial plans. To show how to achieve this with this package, we first need a second conservatio problem and resulting solution. 
...
```{r, fig.width=9}
(ggMPA <- splnr_plot_MPAs(MPAsCoralSea, landmass = landmass)) 
```
...
(ggSoln2 <- splnr_plot_Solution(datEx_soln2, PUs, landmass))
```

#### Visualize comparison

```{r, fig.width=9}
(ggComp <- splnr_plot_comparison(datEx_soln, datEx_soln2, landmass, PlanUnits = PUs))
```
...
datEx_soln_portfolio <- datEx_problem %>%
  prioritizr::add_gap_portfolio(5, 0.2) %>% #create conservation problem that contains a portfolio of solutions
  prioritizr:::solve.ConservationProblem()
...
# MaxDepth <- 200
# 
# AQM <- splnr_get_AquaMaps(PUs, MinDepth, MaxDepth)
```

### Geomorphic features 
Add geomorphic features to the existing features object.
```{r}
# GeoMorph <- splnr_get_Geomorphic(PUs, Feature = c("Shelf", "Ridge", "Plateau", "Seamount"))
```
...
```{r}
# Cutoffs <- 0.5
# AQM_bin <- splnr_Apply_Cutoffs(AQM, Cutoffs)
# 
# Cutoffs <- c(Shelf = 0.01, Ridge = 0.2, Plateau = 0.1, Seamount = 0.3)
# GeoMorph_bin <- splnr_Apply_Cutoffs(GeoMorph, Cutoffs)
```
...
# 
# (ggFeatureNo <- splnr_plot_FeatureNo(features, landmass)) # Plot Number of features
```
