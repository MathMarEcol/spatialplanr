...
```{r}
features <- SpatPlan_Get_AquaMaps(PUs, MinDepth, MaxDepth, CutOff = CO) #TODO Check the overlay of raw v processed species distribution
```
...
```{r}
features <- SpatPlan_Get_Geomorphic(features, Feature = "Seamount")
```
...
To run this code you will need to sign up for your own IUCN RedList token. Start by running `rl_use_iucn()`. 
```{r, eval=FALSE}
speciesCol <- "Species"
Targets <- Targets %>% 
