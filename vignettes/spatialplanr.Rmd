...
```{r}
(ggFeatNo <- splnr_plot(datEx_species_bin, showFeatureSum = TRUE) +
  splnr_gg_add(
    PUs = PUs, Bndry = Bndry, overlay = landmass,
    cropOverlay = PUs, ggtheme = splnr_theme
  ))
...
```{r}
datEx_problem <- prioritizr::problem(out_sf, feature_names, "Apparent.Fishing.Hours") %>%
...
```{r}
(ggSoln <- splnr_plot(datEx_soln, col_names = "solution_1") +
  splnr_gg_add(
    PUs = PUs, Bndry = Bndry, overlay = landmass,
    cropOverlay = PUs, ggtheme = splnr_theme
  ))
...
(ggCostOverlay <- splnr_plot_costOverlay(
  soln = datEx_soln,
  Cost = NA,
  Cost_name = "Apparent.Fishing.Hours"
) +
  splnr_gg_add(
    PUs = PUs, Bndry = Bndry, overlay = landmass,
    cropOverlay = PUs, ggtheme = splnr_theme
