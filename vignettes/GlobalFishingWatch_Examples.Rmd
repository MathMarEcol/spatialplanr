...
the 2.8 million identified fishing vessels). <br>
2. The size of the vessels (52-85% for vessels larger than 24 meters against 1% 
for vessels under 12 meters). <br>
...
1. register for a GFW account here : http://bit.ly/447cRG9 <br>
...
```{r, message=FALSE}
data_sf_combined <- splnr_get_gfw("Australia", 
                                  "2019-01-01", 
                                  "2023-12-31", 
                                  "yearly", 
                                  "low", 
...
main_plot <- ggplot2::ggplot(data_sf_combined) +
  ...
  ggplot2::labs(title = "2022 Vessel Activity Map", subtitle = "Fishing Hours recorded by GFW in Australia", color = "Fishing Hours (log10)") +
  ggplot2::theme(
...
overlay_plot <- ggplot2::ggplot(data_sf_combined) +
  ...
  ggplot2::scale_color_viridis_c(guide = "legend") +
  ggplot2::labs(title = "Vessel Activity Map in Australia between 2019 and 2023", subtitle = "Fishing Hours data recorded by GFW", color = "Fishing Hours \n (log10)") +
  ggplot2::theme_minimal() +
  ggplot2::theme(
...
on AIS operation and limitations here : 
<br> https://globalfishingwatch.org/dataset-and-code-fishing-effort/ <br>
...
https://gateway.api.globalfishingwatch.org/auth?client=gfw&callback=https%3A%2F%2Fglobalfishingwatch.org%2Fmap%2F%3FcallbackUrlStorage%3Dtrue&locale=en 
