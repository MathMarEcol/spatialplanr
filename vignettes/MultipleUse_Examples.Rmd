...
out_sf <- datSpecZone1 %>%
  ...
  sf::st_as_sf() %>%
  dplyr::mutate(Cost1 = runif(n = dim(.)[[1]]) * 100, #when giving sf input, we need as many cost columns as we have zones
...
```{r, fig.width=9}
(gg_s10 <- splnr_plot_solutionZones(s10) +
    splnr_gg_add(PUs = PUs, Bndry = Bndry, land = landmass, cropLand = PUs, ggtheme = splnr_theme))
```
...
```{r}
CostArea <- out_sf %>%
  dplyr::mutate(areaCostZone1 = rep(1,397),
...
```{r, fig.width=9}
(gg_s11 <- splnr_plot_solutionZones(s11) +
    splnr_gg_add(PUs = PUs, Bndry = Bndry, land = landmass, cropLand = PUs, ggtheme = splnr_theme))
```
...
targetAcrossSome <- Dict %>%
  dplyr::mutate(target = dplyr::case_when(Category == "Reptiles"~ 30/100,
                                          Category == "Mammals"~ 10/100,
                                          Category == "Sharks and rays"~ 5/100,
                                          TRUE ~ 0),
                zone = dplyr::case_when(Category == "Reptiles"~ list(c("zone1", "zone2")),
...
```{r}
(gg_s12 <- splnr_plot_solutionZones(s12, 
