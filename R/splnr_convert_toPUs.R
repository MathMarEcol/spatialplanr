...
splnr_convert_toPUs <- function(dat, PlanUnits) {
  ...
  if (stringr::str_detect(class(dat)[1], "SpatRaster") == TRUE) { # Is it a raster or a vector dataset
    ...
    # I think we can use terra::extract now
    # out <- PlanUnits %>%
    ...
    out <- PlanUnits %>%
      dplyr::mutate(!!nm := dplyr::pull(terra::extract(dat, PlanUnits, fun = meth, method = "bilinear", ID = FALSE, weight = TRUE))) # Use mean for continuous. Use mode for categorical
