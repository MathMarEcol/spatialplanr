...
#' @param Type The type of Limits being provided. Options are "Ocean" or "EEZ". (not required if numeric or "Global" limits are provided)
...
splnr_get_boundary <- function(Limits,
  ...
  if (is.numeric(Limits) || Limits == "Global") {
    assertthat::assert_that(missing(Type), msg = "Type must be missing when Limits are numeric or global.")}
