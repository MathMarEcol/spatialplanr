...
#'
#' This is a wrapper for `SpatPlan_Convert2PUs()` but deals with need to processes each layer seperately
#'
...
#' @param cat_name A character string of all categories in the regioalisation
...
#'
#' @examples
SpatPlan_Convert_Regionalisation <- function(dat, PUs, cat_name = NA, col_name = NA){

  if (stringr::str_detect(class(dat)[1], "SpatRaster") == TRUE) { # Rasters

    col_name <- names(dat) # Name of layer

    out <- SpatPlan_Convert_2PUs(terra::as.factor(dat), PUs) %>% # Make categorical first
      SpatPlan_Replace_NAs(col_name) %>% # Remove Na's in variable
